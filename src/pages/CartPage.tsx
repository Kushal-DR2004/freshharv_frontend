...
const CartPage = () => {
    ...
    const navigate = useNavigate()
        ...
        const procedToOrder = async (event: React.FormEvent<HTMLFormElement>) => {
            ...
            try{
                ...
                console.log(13)
                const response = await axios.post(`http://127.0.0.1:8000/api/v1/carts/orders/`, {
                ...
                console.log(product)
               navigate('/order' ,{ state: { id : response?.data?.id , product , address: response?.data?.delivered_address
        ...
        const applyDiscountCode =async (event: React.FormEvent<HTMLFormElement>) => {
            ...
            try{
                ...
                console.log(13)
                const response = await axios.patch(`http://127.0.0.1:8000/api/v1/carts/discountapply/12/`, {
        ...
        const calculate = (cart : Cart) => {
            ...
            cart?.items?.forEach((item) => {total += (parseFloat(item?.product?.product_price)) * item.quantity;
    ...
    const fetchCart = async() => {
            ...
            const response = await axios.get('http://127.0.0.1:8000/api/v1/carts/cartitems/' ,  {
                headers: {
                    Authorization: `Bearer ${token}`, 
                },
            });
  ...
  return (
    <div className='py-5 px-40 w-[1280px]'>
        <section>
            ...
            {
                cart?.items?.map((item) => <CartDetailscomponent key={item.id} id={item.id} product={item.product} quantity={item.quantity} fetchCart={fetchCart}/>)
            }
            
        </section>
        {!cart?.discount_code && cart && cart.items && cart.items.length > 0 && <section>
            <form onSubmit={applyDiscountCode}>
                <div className="p-4 pb-2 font-bold text-[18px] text-[#121712]">Apply Discount Code</div>
                <div className="w-1/2 py-3 px-4">
                    <input className="w-full rounded-lg border border-[#DEE5DB] p-4" placeholder="Enter discount code"
                          value = {discountcode}
                          onChange={handleDiscountChange}
                          required/>
                </div>
                {discountError && <div className="p-4 font-bold text-red-500">{discountError}</div>}
                <div className="py-3 px-4">
                    <button type="submit" className="bg-[#F2F5F0] px-4 py-2 rounded-lg font-bold text-3.5 text-[#121712] focus:border hover:bg-gray-200">Apply</button>
                </div>
                
            </form>
        </section>}
       {cart && cart.items && cart.items.length > 0 && <section>
            <h1 className="p-4 font-bold text-4.5 text-[#121712]">Order Summary</h1>
       ...
       {cart && cart.items && cart.items.length > 0 && <form onSubmit={procedToOrder}>
            ...
            <button type="submit" className="my-3 mx-4 bg-[#4AB814] rounded-lg font-bold text-[14px] text-[#121712] py-3 w-4/10 focus:border hover:bg-green-600">
                    Proceed to Checkout
            </button>
        ...
        <div className="flex justify-center py-4">
            <button className="bg-[#F2F5F0] rounded-lg font-bold text-[14px]  text-[#121712] py-3 w-2/10 focus:border hover:bg-gray-200"
