  ...
  const fetchFarmer = async () => {
    try {
      ...
      const response = await axios.get(
        `http://127.0.0.1:8000/api/v1/farmers/${num.id}`,
        {
          headers: {
    ...
    <div className="rounded-lg p-6 bg-white shadow-lg border border-gray-200 ">
      ...
      <div className="text-center my-2 border-t mb-4">
        <h2 className="text-center font-bold text-2xl text-gray-900">
          {farmer?.farm?.[0].farm_name} -{" "}
          <span className="font-medium text-lg text-[#708763]">
            {farmer?.farm?.[0].location}
          </span>
        </h2>
        <p className="font-medium text-4 text-gray-900 p-2">
          {farmer?.farm?.[0].about}
        </p>
      </div>
      ...
      {farmer?.farm?.[0].farm_products.map((product) => (
        <NavLink to={`/products/${product.id}`}>
          {" "}
          <div className="flex gap-3 rounded-lg p-6 bg-white shadow-lg border border-gray-200 w-1/2 m-6">
