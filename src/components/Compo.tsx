...
const Compo = ({
  ...
  return (
    ...
    >
      <Typography
        sx={{ fontSize: "22px", fontWeight: "bold", px: 4, pt: 5, pb: 3 }}
      >
        Customer Reviews
      </Typography>
      <div className="flex gap-6 p-4">
        <Box>
          ...
          <Box>
            <Rating
              value={(totalreview ? totalreview : 0) * 1.0}
              precision={0.1}
              readOnly
              size="small"
            />
        ...
        >
          {reviewRating?.map((item, i) => (
            ...
            >
              <Typography
                sx={{ fontSize: "14px", fontWeight: "medium", pt: 0.5 }}
              >
                {i + 1}
              </Typography>
              ...
              <Box sx={{ flexGrow: 1, width: "100%" }}>
                <LinearProgress
                  variant="determinate"
                  value={
                    Number.isFinite(item) && (reviewcount ?? 0) > 0
                      ? (item / (reviewcount ?? 1)) * 100
                      : 0
                  }
                  sx={{
                    height: 8,
              ...
              <Typography sx={{ width: 30, textAlign: "right" }}>
                {Number.isFinite(item) && reviewcount
                  ? ((item / reviewcount) * 100).toFixed(0) + "%"
                  : "0%"}
              </Typography>
            </Box>
          ))}
